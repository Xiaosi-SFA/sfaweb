...
在日常开发或内容新增完成后，仓库管理员可直接执行一键发布脚本（包含权限校验、静态构建预检、自动暂存提交、推送当前分支、合并至 `main` 触发 GitHub Actions 部署并安全切回原分支）：

```bash
# 一键自动构建并发布（自动提示输入提交说明）
pnpm ship

# 携带指定提交信息直接发布
pnpm ship "feat: 更新社团动态与文章"

# 跳过本地构建检查直接发布
pnpm ship --skip-build "content: 修正错别字"
```

> **权限控制说明**：该脚本执行前会自动校验当前用户的 GitHub 仓库管理员（ADMIN）权限。非管理员创作者请通过 Pull Request 流程向 `dev` 分支提交稿件。
